...
export function LegacyInteractive() {
  ...
  return <section className="legacy-interactive"><div className="legacy-spark-field" aria-hidden="true"><span className="spark-chip chip-one">AMANAH</span><span className="spark-chip chip-two">DIGNITY</span><span className="spark-chip chip-three">FUTURE</span><span className="spark-line line-one"/><span className="spark-line line-two"/></div><div className="legacy-interactive-copy"><span className="eyebrow">A living legacy</span><div className="legacy-tabs">{Object.keys(panels).map((tab) => <button className={active === tab ? "active" : ""} onClick={() => setActive(tab)} key={tab}>{tab}</button>)}</div><div className="legacy-panel" key={active}><span>{panel.kicker}</span><h2>{panel.title}</h2><p>{panel.body}</p></div><div className="legacy-progress"><span style={{ width: active === "Story" ? "33%" : active === "Values" ? "66%" : "100%" }}/></div></div></section>;
}
