...
export function LegacyInteractive() {
  ...
  return <section className="legacy-path"><div className="legacy-path-heading"><span className="eyebrow">Follow the thread</span><h2>Three moments. One lasting mission.</h2><p>Move through the chapters of Ali’s legacy and see how a personal decision became a global act of service.</p></div><div className="legacy-path-controls">{Object.keys(panels).map((tab, index) => <button className={active === tab ? "active" : ""} onClick={() => setActive(tab)} key={tab}><span>0{index + 1}</span>{tab}<i>↗</i></button>)}</div><article className="legacy-path-card" key={active}><span className="legacy-path-number">0{Object.keys(panels).indexOf(active) + 1}</span><div><span className="eyebrow">{panel.kicker}</span><h3>{panel.title}</h3><p>{panel.body}</p></div><div className="legacy-path-arrow">{active === "Story" ? "→" : active === "Values" ? "↗" : "✦"}</div></article></section>;
}
